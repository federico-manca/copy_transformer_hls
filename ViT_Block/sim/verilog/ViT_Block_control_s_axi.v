...
// ==============================================================
`timescale 1ns/1ps
module ViT_Block_control_s_axi
#(parameter
    ...
    output wire [63:0]                   in_o,
    output wire [63:0]                   in_w_up,
    output wire [63:0]                   in_w_down,
...
localparam
    ...
    ADDR_IER              = 7'h08,
    ADDR_ISR              = 7'h0c,
    ADDR_IN_Q_DATA_0      = 7'h10,
    ADDR_IN_Q_DATA_1      = 7'h14,
    ADDR_IN_Q_CTRL        = 7'h18,
    ADDR_IN_K_DATA_0      = 7'h1c,
    ADDR_IN_K_DATA_1      = 7'h20,
    ADDR_IN_K_CTRL        = 7'h24,
    ADDR_IN_V_DATA_0      = 7'h28,
    ADDR_IN_V_DATA_1      = 7'h2c,
    ADDR_IN_V_CTRL        = 7'h30,
    ADDR_IN_O_DATA_0      = 7'h34,
    ADDR_IN_O_DATA_1      = 7'h38,
    ADDR_IN_O_CTRL        = 7'h3c,
    ADDR_IN_W_UP_DATA_0   = 7'h40,
    ADDR_IN_W_UP_DATA_1   = 7'h44,
    ADDR_IN_W_UP_CTRL     = 7'h48,
    ADDR_IN_W_DOWN_DATA_0 = 7'h4c,
    ADDR_IN_W_DOWN_DATA_1 = 7'h50,
    ADDR_IN_W_DOWN_CTRL   = 7'h54,
    WRIDLE                = 2'd0,
    WRDATA                = 2'd1,
    WRRESP                = 2'd2,
    WRRESET               = 2'd3,
...
//------------------------Local signal-------------------
    ...
    reg  [63:0]                   int_in_o = 'b0;
    reg  [63:0]                   int_in_w_up = 'b0;
    reg  [63:0]                   int_in_w_down = 'b0;
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_IN_O_DATA_0: begin
                    rdata <= int_in_o[31:0];
                end
                ADDR_IN_O_DATA_1: begin
                    rdata <= int_in_o[63:32];
                end
                ADDR_IN_W_UP_DATA_0: begin
                    rdata <= int_in_w_up[31:0];
                end
                ADDR_IN_W_UP_DATA_1: begin
                    rdata <= int_in_w_up[63:32];
                end
                ADDR_IN_W_DOWN_DATA_0: begin
                    rdata <= int_in_w_down[31:0];
                end
                ADDR_IN_W_DOWN_DATA_1: begin
                    rdata <= int_in_w_down[63:32];
                end
            endcase
        end
    end
...
assign in_o              = int_in_o;
assign in_w_up           = int_in_w_up;
assign in_w_down         = int_in_w_down;
// int_interrupt
always @(posedge ACLK) begin
    if (ARESET)
        int_interrupt <= 1'b0;
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_IN_O_DATA_0)
            int_in_o[31:0] <= (WDATA[31:0] & wmask) | (int_in_o[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_IN_O_DATA_1)
            int_in_o[63:32] <= (WDATA[31:0] & wmask) | (int_in_o[63:32] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_IN_W_UP_DATA_0)
            int_in_w_up[31:0] <= (WDATA[31:0] & wmask) | (int_in_w_up[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_IN_W_UP_DATA_1)
            int_in_w_up[63:32] <= (WDATA[31:0] & wmask) | (int_in_w_up[63:32] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        int_in_w_down[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_IN_W_DOWN_DATA_0)
            int_in_w_down[31:0] <= (WDATA[31:0] & wmask) | (int_in_w_down[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        int_in_w_down[63:32] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_IN_W_DOWN_DATA_1)
            int_in_w_down[63:32] <= (WDATA[31:0] & wmask) | (int_in_w_down[63:32] & ~wmask);
    end
end
